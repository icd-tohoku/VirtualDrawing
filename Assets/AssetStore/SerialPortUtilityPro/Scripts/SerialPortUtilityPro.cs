...
namespace SerialPortUtility
{
	...
	{
        //Version Infomation
        public const string VersionString = "2.33";
			...
			if(_androidPlugin != null)
			{
				...
				switch (OpenMethod)	//Android
				{
					...
					case SerialPortUtilityPro.OpenSystem.PCI:
						SerialPortHandle = _androidPlugin.Call<int>("OpenPCI", VendorID, config);
						break;
		...
		public void ReadUpdate()
		{
			...
			if (size > 0)
			{
				...
				if(_androidPlugin != null) {
					...
					//dataArray = System.Array.ConvertAll(_androidPlugin.Get<sbyte[]>("m_byteData"), b => unchecked((byte)b));
					sbyte[] dataArray_signed = _androidPlugin.Get<sbyte[]>("m_byteData");
					dataArray = new byte[dataArray_signed.Length];
					System.Buffer.BlockCopy(dataArray_signed, 0, dataArray, 0, dataArray_signed.Length);
				}
#else
        ...
        public static DeviceInfo[] GetConnectedDeviceList(OpenSystem device_format)
        {
			...
			else if (device_format == OpenSystem.BluetoothSSP)
			{
                ...
                if (androidPlugin != null) {
					...
					{
						deviceInfo[i] = new DeviceInfo();
						deviceInfo[i].SerialNumber = deviceKind[i];    //SerialNumber
					}
				}
			}
            else
